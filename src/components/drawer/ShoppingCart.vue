...
<template>
    <div v-if="shoppingCart.getNumberOfItems === 0" class="flex flex-col items-center text-center gap-3 mt-40">
        ...
        <button @click="drawer.close()" class="btn btn-primary btn-block">Start shopping</button>
    </div>

    <div v-else class="flex flex-col gap-4 mt-8">
        <div v-for="[restaurant, items] in shoppingCart.getCheckoutSummary.restaurantNameItemsMap" :key="restaurant">
